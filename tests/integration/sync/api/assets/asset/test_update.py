def test_update_asset(plytix, new_asset_data_from_url_factory):
    ...
    result = plytix.assets.update_asset(asset.id, new_filename)

    assert result.filename == new_filename


def test_update_asset_not_found(plytix):
    result = plytix.assets.update_asset("non-existing-id", "new-filename")

    assert result is None


def test_update_multiple_assets(plytix, new_asset_data_from_url_factory):
    ...
    results = plytix.assets.update_assets(updated_assets)
