def test_get_family(client, new_family_data):
    result = client.families.create_family(**new_family_data)

    result = client.families.get_family(result.id)

    assert result.name == new_family_data["name"]


def test_get_family_not_found(client):
    result = client.families.get_family("non-existing-id")

    assert result is None


def test_get_multiple_families(client, new_family_data):
    ...
    results = client.families.get_families(family_ids)
