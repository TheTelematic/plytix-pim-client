def test_rename_family(client, new_family_data):
    result = client.families.create_family(**new_family_data)

    new_name = f"{new_family_data['name']}-new"
    result = client.families.rename_family(result.id, new_name)

    assert result.name == new_name


def test_rename_family_not_found(client):
    result = client.families.rename_family("non-existing-id", "new name")

    assert result is None


def test_update_multiple_families(client, new_family_data):
    ...
    results = client.families.rename_families(updated_families)
