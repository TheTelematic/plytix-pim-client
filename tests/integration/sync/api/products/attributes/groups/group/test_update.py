def test_update_attributes_group(plytix, new_product_attributes_group_data):
    group = plytix.products.attributes.groups.create_attributes_group(**new_product_attributes_group_data)

    updated_attribute = plytix.products.attributes.groups.update_attributes_group(
        group.id,
...
def test_update_not_existing_attribute(plytix):
    updated_group = plytix.products.attributes.groups.update_attributes_group(
...
def test_update_attributes_groups(plytix, new_product_attributes_group_data):
    ...
    group1 = plytix.products.attributes.groups.create_attributes_group(**group1)
    group2 = plytix.products.attributes.groups.create_attributes_group(**group2)
    assert group1.id is not None
    assert group2.id is not None

    updated_attributes = plytix.products.attributes.groups.update_attributes_groups(
        [
