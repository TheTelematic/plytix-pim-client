def test_assign_product_to_family(client, new_product_data, new_product_family_data):
    ...
    result = client.products.assign_family(product.id, family.id)
...
def test_assign_product_to_family_not_found(client, new_product_data):
    ...
    result = client.products.assign_family(product.id, "non-existing-id")

    assert result is None


def test_assign_product_to_family_product_not_found(client, new_product_family_data):
    family = client.products.families.create_family(**new_product_family_data)

    result = client.products.assign_family("non-existing-id", family.id)

    assert result is None


def test_assign_multiple_products_to_family(client, new_product_data, new_product_family_data):
    ...
    results = client.products.assign_families([(product1.id, family.id), (product2.id, family.id)])
