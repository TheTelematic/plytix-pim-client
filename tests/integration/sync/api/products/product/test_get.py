def test_get_product(client, new_product_data):
    result = client.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = client.products.get_product(result.id)
...
def test_get_product_not_found(client):
    result = client.products.get_product("non-existing-id")

    assert result is None


def test_get_multiple_products(client, new_product_data):
    ...
    results = client.products.get_products(product_ids)
