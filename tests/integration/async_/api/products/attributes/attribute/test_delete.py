async def test_delete_attribute(client, new_product_attribute_data):
    ...
    deleted = await client.products.attributes.delete_attribute(product_attribute.id)
    assert deleted

    product_attribute = await client.products.attributes.get_attribute(product_attribute.id)
    assert product_attribute is None


async def test_delete_attribute_that_does_not_exist(client):
    deleted = await client.products.attributes.delete_attribute("non_existent_id")
    assert not deleted


async def test_delete_attributes(client, new_product_attribute_data):
    ...
    deleted = await client.products.attributes.delete_attributes(
        [product_attribute.id for product_attribute in product_attributes]
    )
    assert all(deleted)

    product_attributes = await client.products.attributes.get_attributes(
        [product_attribute.id for product_attribute in product_attributes]
    )
