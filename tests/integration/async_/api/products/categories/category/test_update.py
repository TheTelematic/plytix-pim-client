...
async def test_convert_to_first_level_category(plytix, new_product_category_data):
    ...
    parent_category = await plytix.products.categories.create_product_category(**parent_category)
    subcategory = await plytix.products.categories.create_product_category(
        parent_category_id=parent_category.id, **subcategory
    )

    subcategory = await plytix.products.categories.convert_to_first_level_category(subcategory.id)

    assert subcategory.parents_ids == []


async def test_move_category(plytix, new_product_category_data):
    parent_category = new_product_category_data.copy()
    subcategory = new_product_category_data.copy()
    parent_category["name"] = f"{parent_category['name']}-parent"
    subcategory["name"] = f"{subcategory['name']}-sub"
    ...
    new_parent_category = await plytix.products.categories.create_product_category(**new_product_category_data)

    subcategory = await plytix.products.categories.move_category(subcategory.id, new_parent_category.id)
...
async def test_convert_to_first_level_multiple_categories(plytix, new_product_category_data):
    ...
    parent_category = await plytix.products.categories.create_product_category(**parent_category)
    subcategory1 = await plytix.products.categories.create_product_category(
        parent_category_id=parent_category.id, **subcategory1
    )
    subcategory2 = await plytix.products.categories.create_product_category(
        parent_category_id=parent_category.id, **subcategory2
    )

    subcategories = await plytix.products.categories.convert_to_first_level_categories(
...
async def test_move_multiple_categories(plytix, new_product_category_data):
    ...
    subcategory1["name"] = f"{subcategory1['name']}-sub1"
    subcategory2["name"] = f"{subcategory2['name']}-sub2"
    parent_category1 = await plytix.products.categories.create_product_category(**parent_category1)
    parent_category2 = await plytix.products.categories.create_product_category(**parent_category2)
    subcategory1 = await plytix.products.categories.create_product_category(
    ...
    subcategories = await plytix.products.categories.move_categories(
