async def test_create_product_category(plytix, new_product_category_data):
...
async def test_create_product_subcategory(plytix, new_product_category_data):
    parent_category = await plytix.products.categories.create_product_category(**new_product_category_data)
...
async def test_create_product_categories(plytix, new_product_category_data):
    ...
    product_categories = await plytix.products.categories.create_product_categories([category1, category2])
    assert product_categories[0].name == category1["name"]
    assert product_categories[1].name == category2["name"]


async def test_create_product_subcategories(plytix, new_product_category_data):
    parent_category = await plytix.products.categories.create_product_category(**new_product_category_data)
    ...
    subcategories = await plytix.products.categories.create_product_categories([subcategory1_data, subcategory2_data])
