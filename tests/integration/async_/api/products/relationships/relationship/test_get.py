async def test_get_relationship(plytix, new_product_relationship_data):
    product_relationship = await plytix.products.relationships.create_product_relationship(
        **new_product_relationship_data
    )

    retrieved_product_relationship = await plytix.products.relationships.get_relationship(product_relationship.id)
...
async def test_get_relationship_that_does_not_exist(plytix):
    product_relationship = await plytix.products.relationships.get_relationship("non_existent_id")
    assert product_relationship is None


async def test_get_relationships(plytix, new_product_relationship_data):
    ...
    product_relationships = [
        await plytix.products.relationships.create_product_relationship(**relationship1),
        await plytix.products.relationships.create_product_relationship(**relationship2),
        await plytix.products.relationships.create_product_relationship(**relationship3),
    ]

    retrieved_product_relationships = await plytix.products.relationships.get_relationships(
