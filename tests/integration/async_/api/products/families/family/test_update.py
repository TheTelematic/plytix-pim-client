async def test_rename_family(plytix, new_product_family_data):
    result = await plytix.products.families.create_family(**new_product_family_data)

    new_name = f"{new_product_family_data['name']}-new"
    result = await plytix.products.families.rename_family(result.id, new_name)

    assert result.name == new_name


async def test_rename_family_not_found(plytix):
    result = await plytix.products.families.rename_family("non-existing-id", "new name")

    assert result is None


async def test_update_multiple_families(plytix, new_product_family_data):
    ...
    results = await plytix.products.families.rename_families(updated_families)
