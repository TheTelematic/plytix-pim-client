async def test_delete_product(client, new_product_data):
    result = await client.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = await client.products.delete_product(result.id)

    assert result is True


async def test_delete_product_not_found(client):
    result = await client.products.delete_product("non-existing-id")

    assert result is False


async def test_delete_multiple_products(client, new_product_data):
    ...
    results = await client.products.delete_products(product_ids)
