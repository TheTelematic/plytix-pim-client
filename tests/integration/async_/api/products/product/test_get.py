async def test_get_product(client, new_product_data):
    result = await client.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = await client.products.get_product(result.id)
...
async def test_get_product_not_found(client):
    result = await client.products.get_product("non-existing-id")

    assert result is None


async def test_get_multiple_products(client, new_product_data):
    ...
    results = await client.products.get_products(product_ids)
