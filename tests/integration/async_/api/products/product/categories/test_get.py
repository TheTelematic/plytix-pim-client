async def test_get_one_category(plytix, product, product_category):
    await plytix.products.categories.link_product_to_category(product.id, product_category.id)

    categories = await plytix.products.categories.get_product_categories(product.id, product_category.id)
...
async def test_get_one_category_of_multiple_products(
    ...
    categories_groups = await plytix.products.categories.get_multiple_product_categories(
        [
            (product.id, product_category.id),
            (product2.id, product_category2.id),
        ]
...
async def test_get_not_existing_category(plytix):
    categories = await plytix.products.categories.get_product_categories("not-existing", "not-existing")

    assert categories is None


async def test_get_not_existing_category_of_multiple_products(plytix):
    categories_groups = await plytix.products.categories.get_multiple_product_categories(
        [
...
async def test_get_all_categories_for_one_product(plytix, product, product_category, new_product_category_data):
    product_category2 = new_product_category_data.copy()
    product_category2["name"] = f"{product_category2['name']}-2"
    product_category2 = await plytix.products.categories.create_product_category(**product_category2)
    await plytix.products.categories.link_product_to_categories(
    ...
    categories = await plytix.products.categories.get_product_categories(product.id)

    assert len(categories) == 2
    assert categories[0].id == product_category.id or product_category2.id
    assert categories[0].name == product_category.name or product_category2.name
    assert categories[0].path == product_category.path or product_category2.path
    assert categories[1].id == product_category.id or product_category2.id
    assert categories[1].name == product_category.name or product_category2.name
    assert categories[1].path == product_category.path or product_category2.path
