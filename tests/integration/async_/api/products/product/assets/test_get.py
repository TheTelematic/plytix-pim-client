...
async def test_get_one_asset(plytix, product, asset, product_attribute_media):
...
async def test_get_all_assets_for_one_product(
    ...
    product_attribute_media2 = await plytix.products.attributes.create_attribute(**new_product_attribute_data)
    asset2 = await plytix.assets.create_asset_by_url(**new_asset_data_from_url_factory())
    await plytix.products.assets.link_asset_to_products(
        [
            (product.id, asset.id, product_attribute_media.label),
