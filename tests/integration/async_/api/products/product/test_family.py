async def test_assign_product_to_family(client, new_product_data, new_family_data):
    product = await client.products.create_product(**new_product_data)
    family = await client.families.create_family(**new_family_data)

    result = await client.products.assign_family_to_product(product.id, family.id)
...
async def test_assign_product_to_family_not_found(client, new_product_data):
    ...
    result = await client.products.assign_family_to_product(product.id, "non-existing-id")

    assert result is None


async def test_assign_product_to_family_product_not_found(client, new_family_data):
    family = await client.families.create_family(**new_family_data)

    result = await client.products.assign_family_to_product("non-existing-id", family.id)

    assert result is None


async def test_assign_multiple_products_to_family(client, new_product_data, new_family_data):
    ...
    results = await client.products.assign_family_to_products([(product1.id, family.id), (product2.id, family.id)])
