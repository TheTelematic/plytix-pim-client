...
async def test_search_assets(plytix, new_asset_data_from_url_factory):
    ...
    search_results_2 = await plytix.assets.search_assets(
        filters=[[SearchFilter(field="id", operator=OperatorEnum.EQUALS, value=assets[2].id)]],
        attributes=[],
        relationship_filters=[],
        pagination=Pagination(page=1, page_size=10, sort_by_attribute="id", sort_ascending=True),
