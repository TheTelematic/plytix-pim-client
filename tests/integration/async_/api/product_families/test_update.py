async def test_rename_family(client, new_family_data):
    result = await client.families.create_family(**new_family_data)

    new_name = f"{new_family_data['name']}-new"
    result = await client.families.rename_family(result.id, new_name)

    assert result.name == new_name


async def test_rename_family_not_found(client):
    result = await client.families.rename_family("non-existing-id", "new name")

    assert result is None


async def test_update_multiple_families(client, new_family_data):
    ...
    results = await client.families.rename_families(updated_families)
