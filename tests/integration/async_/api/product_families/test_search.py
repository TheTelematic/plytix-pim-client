...
async def test_search_families(client, new_family_data):
    ...
    await client.families.create_families(families)

    search_results = await client.families.search_families(
...
async def test_search_all_families(client, new_family_data):
    new_family_data_1 = new_family_data.copy()
    new_family_data_2 = new_family_data.copy()
    new_family_data_3 = new_family_data.copy()
    ...
    search_results = []
    async for families in client.families.search_all_families(
        filters=[[SearchFilter(field="name", operator=OperatorEnum.CONTAINS, value=new_family_data["name"])]],
