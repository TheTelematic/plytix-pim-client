def test_get_product(plytix, new_product_data):
    result = plytix.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = plytix.products.get_product(result.id)

    assert result.sku == new_product_data["sku"]
    assert result.label == new_product_data["label"]


def test_get_product_not_found(plytix):
    result = plytix.products.get_product("non-existing-id")

    assert result is None


def test_get_multiple_products(plytix, new_product_data):
    ...
    results = plytix.products.get_products(product_ids)
