...
def test_unlink_relationship_to_product(plytix, product, product_relationship, new_product_data):
    ...
    result = plytix.products.relationships.unlink_product_from_relationship(
...
@pytest.mark.skip("To be fixed in https://github.com/TheTelematic/plytix-pim-client/issues/44")
def test_multiple_products_to_relationships(
    plytix, product, product_relationship, new_product_data, new_product_relationship_data
):
    ...
    plytix.products.relationships.link_product_to_relationships(
        [
            {
    ...
    results = plytix.products.relationships.unlink_product_from_relationships(
        [
            {
                "product_id": product.id,
                "product_relationship_id": product_relationship.id,
