async def test_get_family(plytix_factory, new_product_family_data):
    result = await plytix_factory.products.families.create_family(**new_product_family_data)

    result = await plytix_factory.products.families.get_family(result.id)

    assert result.name == new_product_family_data["name"]


async def test_get_family_not_found(plytix_factory):
    result = await plytix_factory.products.families.get_family("non-existing-id")

    assert result is None


async def test_get_multiple_families(plytix_factory, new_product_family_data):
    ...
    results = await plytix_factory.products.families.get_families(family_ids)
