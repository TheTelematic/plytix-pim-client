...
async def test_get_family_attributes(plytix_factory, new_product_family_data, new_product_attribute_data):
    family = await plytix_factory.products.families.create_family(**new_product_family_data)
    attribute = await plytix_factory.products.attributes.create_attribute(**new_product_attribute_data)
    await plytix_factory.products.families.attributes.link_attribute_to_family(family.id, [attribute.id])

    results = await plytix_factory.products.families.attributes.get_family_attributes(family.id)
...
async def test_get_family_attributes_not_found(plytix_factory):
    results = await plytix_factory.products.families.attributes.get_family_attributes("not-found")
    assert results is None


async def test_get_families_attributes(plytix_factory, new_product_family_data, new_product_attribute_data):
    ...
    family1 = await plytix_factory.products.families.create_family(**family1)
    family2 = await plytix_factory.products.families.create_family(**family2)
    ...
    await plytix_factory.products.families.attributes.link_attribute_to_family(family1.id, [attribute1.id])
    await plytix_factory.products.families.attributes.link_attribute_to_family(family2.id, [attribute2.id])

    results = await plytix_factory.products.families.attributes.get_families_attributes([family1.id, family2.id])
