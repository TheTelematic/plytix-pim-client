async def test_rename_family(plytix_factory, new_product_family_data):
    ...
    result = await plytix_factory.products.families.rename_family(result.id, new_name)

    assert result.name == new_name


async def test_rename_family_not_found(plytix_factory):
    result = await plytix_factory.products.families.rename_family("non-existing-id", "new name")

    assert result is None


async def test_update_multiple_families(plytix_factory, new_product_family_data):
    ...
    results = await plytix_factory.products.families.rename_families(updated_families)
