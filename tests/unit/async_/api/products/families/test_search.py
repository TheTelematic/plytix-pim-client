...
async def test_search_families(plytix_factory, new_product_family_data):
    ...
    await plytix_factory.products.families.create_families(families)

    search_results = await plytix_factory.products.families.search_families(
...
async def test_search_all_families(plytix_factory, new_product_family_data):
    new_family_data_1 = new_product_family_data.copy()
    new_family_data_2 = new_product_family_data.copy()
    new_family_data_3 = new_product_family_data.copy()
    ...
    search_results = []
    async for families in plytix_factory.products.families.search_all_families(
        filters=[[SearchFilter(field="name", operator=OperatorEnum.CONTAINS, value=new_product_family_data["name"])]],
