...
async def test_search_product_relationships(plytix_factory, new_product_relationship_data):
    ...
    relationships = await plytix_factory.products.relationships.create_product_relationships(
        [new_product_data_1, new_product_data_2, new_product_data_3]
    )

    search_results = await plytix_factory.products.relationships.search_product_relationships(
        filters=[
            [
                SearchFilter(
...
async def test_search_all_product_relationships(plytix_factory, new_product_relationship_data):
    new_product_data_1 = new_product_relationship_data.copy()
    new_product_data_2 = new_product_relationship_data.copy()
    new_product_data_3 = new_product_relationship_data.copy()
    new_product_data_1["name"] = f"{new_product_data_1['name']}-1"
    ...
    search_results = []
    async for results in plytix_factory.products.relationships.search_all_product_relationships(
        filters=[
            [
                SearchFilter(
                    field="id", operator=OperatorEnum.IN, value=[relationship.id for relationship in relationships]
