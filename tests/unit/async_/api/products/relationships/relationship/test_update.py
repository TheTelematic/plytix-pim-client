async def test_update_relationship(plytix_factory, new_product_relationship_data):
    relationship = await plytix_factory.products.relationships.create_product_relationship(**new_product_relationship_data)

    updated_relationship = await plytix_factory.products.relationships.update_relationship(
        relationship.id,
...
async def test_update_not_existing_relationship(plytix_factory):
    updated_relationship = await plytix_factory.products.relationships.update_relationship(
...
async def test_update_relationships(plytix_factory, new_product_relationship_data):
    ...
    updated_relationships = await plytix_factory.products.relationships.update_relationships(
        [
