async def test_create_product_ok(plytix_factory, new_product_data):
...
async def test_create_product_no_label(plytix_factory, new_product_data):
    result = await plytix_factory.products.create_product(new_product_data["sku"])

    assert result.sku == new_product_data["sku"]
    assert result.label is None


async def test_create_multiple_products(plytix_factory, new_product_data):
