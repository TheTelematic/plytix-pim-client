async def test_get_product(plytix_factory, new_product_data):
    result = await plytix_factory.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = await plytix_factory.products.get_product(result.id)

    assert result.sku == new_product_data["sku"]
    assert result.label == new_product_data["label"]


async def test_get_product_not_found(plytix_factory):
    result = await plytix_factory.products.get_product("non-existing-id")

    assert result is None


async def test_get_multiple_products(plytix_factory, new_product_data):
    ...
    results = await plytix_factory.products.get_products(product_ids)
