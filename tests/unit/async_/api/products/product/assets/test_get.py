...
async def test_get_one_asset(plytix_factory, product, asset, product_attribute_media):
    await plytix_factory.products.assets.link_asset_to_product(product.id, asset.id, product_attribute_media.label)

    assets = await plytix_factory.products.assets.get_product_assets(product.id, asset.id)
...
async def test_get_one_asset_of_multiple_products(
        plytix_factory, product, asset, new_product_data, new_asset_data_from_url_factory, product_attribute_media
):
    ...
    assets_groups = await plytix_factory.products.assets.get_multiple_product_assets(
        [
            (product.id, asset.id),
...
async def test_get_not_existing_asset(plytix_factory):
    assets = await plytix_factory.products.assets.get_product_assets("not-existing", "not-existing")

    assert assets is None


async def test_get_not_existing_asset_of_multiple_products(plytix_factory):
    assets_groups = await plytix_factory.products.assets.get_multiple_product_assets(
        [
...
async def test_get_all_assets_for_one_product(
        plytix_factory, product, asset, new_asset_data_from_url_factory, product_attribute_media, new_product_attribute_data
):
    ...
    product_attribute_media2 = await plytix_factory.products.attributes.create_attribute(**new_product_attribute_data)
    asset2 = await plytix_factory.assets.create_asset_by_url(**new_asset_data_from_url_factory())
    await plytix_factory.products.assets.link_asset_to_products(
        [
            (product.id, asset.id, product_attribute_media.label),
    ...
    assets = await plytix_factory.products.assets.get_product_assets(product.id)
