async def test_link_asset_to_product(plytix_factory, product, asset, product_attribute_media):
    result = await plytix_factory.products.assets.link_asset_to_product(product.id, asset.id, product_attribute_media.label)
...
async def test_link_not_existing_product(plytix_factory):
    result = await plytix_factory.products.assets.link_asset_to_product("not-existing", "not-existing", "not-existing")
...
async def test_link_multiple_asset_to_products(
    ...
    results = await plytix_factory.products.assets.link_asset_to_products(
        [
