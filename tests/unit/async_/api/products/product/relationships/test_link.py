async def test_link_product_to_relationship(plytix_factory, product, product_relationship, new_product_data):
    ...
    relationship = await plytix_factory.products.relationships.link_product_to_relationship(
        product.id,
...
async def test_link_product_to_relationships_not_found(plytix_factory):
    relationship = await plytix_factory.products.relationships.link_product_to_relationship(
        "not-found",
        "not-found",
...
async def test_link_products_to_relationships(plytix_factory, product, product_relationship, new_product_data):
    ...
    relationships = await plytix_factory.products.relationships.link_product_to_relationships(
        [
            {
