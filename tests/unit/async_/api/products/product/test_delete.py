async def test_delete_product(plytix_factory, new_product_data):
    result = await plytix_factory.products.create_product(new_product_data["sku"], new_product_data["label"])

    result = await plytix_factory.products.delete_product(result.id)

    assert result is True


async def test_delete_product_not_found(plytix_factory):
    result = await plytix_factory.products.delete_product("non-existing-id")

    assert result is False


async def test_delete_multiple_products(plytix_factory, new_product_data):
    ...
    results = await plytix_factory.products.delete_products(product_ids)
