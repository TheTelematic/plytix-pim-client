async def test_update_attributes_group(plytix_factory, new_product_attributes_group_data):
    group = await plytix_factory.products.attributes.groups.create_attributes_group(**new_product_attributes_group_data)

    updated_attribute = await plytix_factory.products.attributes.groups.update_attributes_group(
        group.id,
...
async def test_update_not_existing_attribute(plytix_factory):
    updated_group = await plytix_factory.products.attributes.groups.update_attributes_group(
...
async def test_update_attributes_groups(plytix_factory, new_product_attributes_group_data):
    ...
    group1 = await plytix_factory.products.attributes.groups.create_attributes_group(**group1)
    group2 = await plytix_factory.products.attributes.groups.create_attributes_group(**group2)
    assert group1.id is not None
    assert group2.id is not None

    updated_attributes = await plytix_factory.products.attributes.groups.update_attributes_groups(
        [
