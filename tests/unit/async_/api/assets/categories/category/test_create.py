async def test_create_asset_category(plytix_factory, new_asset_category_data):
...
async def test_create_asset_subcategory(plytix_factory, new_asset_category_data):
    parent_category = await plytix_factory.assets.categories.create_asset_category(**new_asset_category_data)
...
async def test_create_asset_categories(plytix_factory, new_asset_category_data):
    ...
    asset_categories = await plytix_factory.assets.categories.create_asset_categories([category1, category2])
    assert asset_categories[0].name == category1["name"]
    assert asset_categories[1].name == category2["name"]


async def test_create_asset_subcategories(plytix_factory, new_asset_category_data):
    parent_category = await plytix_factory.assets.categories.create_asset_category(**new_asset_category_data)
    ...
    subcategories = await plytix_factory.assets.categories.create_asset_categories([subcategory1_data, subcategory2_data])
