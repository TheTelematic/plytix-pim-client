async def test_update_asset(plytix_factory, new_asset_data_from_url_factory):
    ...
    result = await plytix_factory.assets.update_asset(asset.id, new_filename)

    assert result.filename == new_filename


async def test_update_asset_not_found(plytix_factory):
    result = await plytix_factory.assets.update_asset("non-existing-id", "new-filename")

    assert result is None


async def test_update_multiple_assets(plytix_factory, new_asset_data_from_url_factory):
    ...
    results = await plytix_factory.assets.update_assets(updated_assets)
